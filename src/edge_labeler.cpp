...
namespace g2o {
  ...
  bool EdgeLabeler::computePartialInverse(SparseBlockMatrix<MatrixXd>& spinv, const std::set<std::pair<int,int> >& pattern){
    ...
    for(std::set<std::pair<int, int> >::const_iterator it= pattern.begin(); it!=pattern.end(); it++){
      blockIndices[k]=*it;
      //cerr <<"(" << blockIndices[k].first << "," << blockIndices[k].second <<")";
      k++;
    }
    //cerr <<  endl;
  ...
  bool EdgeLabeler::labelEdge( const SparseBlockMatrix<MatrixXd>& spinv, OptimizableGraph::Edge* e){
	...
	for (size_t j=0; j<e->vertices().size(); j++){
	  ...
	  if (tj>-1){ 
	    ...
	    if(spinv.block(ti,tj) == 0 && spinv.block(tj, ti) == 0){
	      OptimizableGraph::Vertex* gaug= (OptimizableGraph::Vertex*)(e->vertices()[0]);
	      //std::cerr << "gauge= " << gaug->hessianIndex() << " ti = " << ti << ", tj=" << tj << std::endl;
	      //std::cerr << "both spinv.block(ti,tj) and spinv.block(tj,ti) are 0" << std::endl;
	    }
	    if (ti<=tj){

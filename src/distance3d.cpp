...
// global variables
pthread_mutex_t _mutex_total_error;
pthread_mutex_t _mutex_best_error;
...
double getError(g2o::HyperGraph::VertexIDMap &map1, g2o::HyperGraph::VertexIDMap &map2, g2o::OptimizableGraph::Vertex * v){
  ...
  g2o::VertexSE3 * v1 = dynamic_cast<g2o::VertexSE3 *>(v);
  
  if(v1 == NULL){
    // it's a landmark
    g2o::VertexPointXYZ * v1 = dynamic_cast<g2o::VertexPointXYZ *>(v);
    if(v1==NULL){
      std::cerr << "Vertex " << v->id() << " isn't a VertexSE3 nor a VertexPointXYZ. Cannot handle it. I'm out!" << std::endl;
      exit(8);
    }
    g2o::VertexPointXYZ * v2 = (g2o::VertexPointXYZ *)map2[v1->id()];
    v1_est[0] = v1->estimate()[0];
    v1_est[1] = v1->estimate()[1];
    v1_est[2] = v1->estimate()[2];
    v2_est[0] = v2->estimate()[0];
    v2_est[1] = v2->estimate()[1];
    v2_est[2] = v2->estimate()[2];
  }
  else{
    g2o::VertexSE3 * v2 = (g2o::VertexSE3 *)map2[v1->id()];
    v1->getMinimalEstimateData(v1_est);
    v2->getMinimalEstimateData(v2_est);
  }
  ...
  for(g2o::HyperGraph::VertexIDMap::iterator it=map1.begin(); it!=map1.end(); it++){
    ...
    g2o::VertexSE3 * p1 = dynamic_cast<g2o::VertexSE3 *>(w1);
    
    if(p1!=NULL){ // it's a pose
      g2o::VertexSE3 * p2 = (g2o::VertexSE3 *) map2[p1->id()];
      
      p1->getMinimalEstimateData(w1_est);
      p2->getMinimalEstimateData(w2_est);
    }
    else{ // it is a landmark
      g2o::VertexPointXYZ * p1 = dynamic_cast<g2o::VertexPointXYZ *>(w1);
	...
	exit(7);
      ...
      g2o::VertexPointXYZ * p2 = (g2o::VertexPointXYZ *) map2[p1->id()];
      
      w1_est[0] = p1->estimate()[0];
      w1_est[1] = p1->estimate()[1];
      w1_est[2] = p1->estimate()[2];
      w2_est[0] = p2->estimate()[0];
      w2_est[1] = p2->estimate()[1];
      w2_est[2] = p2->estimate()[2];
    }
    
    double dist1 = sqrt(pow(w1_est[0] - v1_est[0], 2) + pow(w1_est[1] - v1_est[1], 2) + pow(w1_est[2] - v1_est[2], 2));
    double dist2 = sqrt(pow(w2_est[0] - v2_est[0], 2) + pow(w2_est[1] - v2_est[1], 2) + pow(w2_est[2] - v2_est[2], 2));
 ...
 void checkBestError(double error, int id){
   pthread_mutex_lock(&_mutex_best_error);
   ...
   pthread_mutex_unlock(&_mutex_best_error);
 }
...
void analyze(g2o::HyperGraph::VertexIDMap &map1, g2o::HyperGraph::VertexIDMap &map2, std::vector<g2o::OptimizableGraph::Vertex *> &vertices, bool*visited){
  ...
  while(index < vertices.size()){
    ...
    pthread_mutex_lock(&_mutex_total_error);
    _total_error += error;
    pthread_mutex_unlock(&_mutex_total_error);
    
    checkBestError(error, v->id());
...
void visualize(bool * visited, unsigned int size){
  ...
  while(index < size){
    ...
    index++;
    if(evolved){
      std::cout << "progress: " << floor(((double)index/(double)(size-1))*10000+0.5)/100 << " %" << std::endl;
    }
    evolved = false;
  }
}
...
void init(){
  ...
  if(pthread_mutex_init(&_mutex_total_error, NULL) == -1){
    std::cout << "errors initializing mutexes" << std::endl;
    exit(9);
  }
  if(pthread_mutex_init(&_mutex_best_error, NULL) == -1){
    std::cout << "errors initializing mutexes" << std::endl;
    exit(9);
  }
...
int main(int argc, char ** argv){
  ...
  std::cout << "minimum error = " << _best_error << std::endl;
